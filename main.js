...
function queryWikiApi(author) {
  ...
  // Do the below as a fallback if there is no author, usually stuff that would get called later on in the queue
  // The learn more button should be set to disabled if there is no author and it is not already disabled
  } else {
    if (!learnMoreButton.classList.contains('disabled')) {
      learnMoreButton.classList.add('disabled');
    }

    // changeColor() is called after querying the wiki api for a synchronised transition. If there is no author it won't get called. Invoke it here in case of no author
    changeColor();
  }
}
...
function learnMoreState(author, pageExists) {
  ...
  // Disable the button if any of the above conditions are met and the button is currently not disabled. Also, remove any added styling to the backgroundColor property
...
function changeColor() {
  ...
  // Change the color of any non-disabled buttons, and remove any previous coloring of disabled buttons
...
// Event listeners for the hide and un-hide buttons. For testing/de-bugging
// document.getElementById('hide').addEventListener('click', e => {
//   slideUp();
// });
//
// document.getElementById('unhide').addEventListener('click', e => {
//   slideDown();
// });
